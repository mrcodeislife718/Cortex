...
const DEFAULT_PROVIDER_PRICING = Object.freeze({
  OPENAI: Object.freeze({ input: 4, output: 20 }),
  ANTHROPIC: Object.freeze({ input: 2, output: 10 }),
  GEMINI: Object.freeze({ input: 0.75, output: 3.75 }),
});

export class CortexCommercialRuntime {
  constructor({ oidc, sessions, stripe, webhookVerifier, repository, entitlements = new EntitlementService(), modelRuntime = null, orchestrator = new AssistantOrchestrator(), promptBoundary = new PromptBoundary(), commercialConfig = {} } = {}) {
    ...
    return new CortexCommercialRuntime({
      ...
      commercialConfig: {
        ...
        monthlyAiBudgetByPlan: {
          pro: optionalPositiveNumber(env.CORTEX_PRO_AI_MONTHLY_BUDGET_USD),
          team: optionalPositiveNumber(env.CORTEX_TEAM_AI_MONTHLY_BUDGET_USD),
          enterprise: optionalPositiveNumber(env.CORTEX_ENTERPRISE_AI_MONTHLY_BUDGET_USD),
        },
      },
    });
  }
  ...
  async assistant({ session, input, context = [], budgetUsd = null }) {
    const subscription = await this.repository.get(session.sub);
    ...
    const planBudget = this.commercialConfig.monthlyAiBudgetByPlan[subscription.plan];
    let monthSpendUsd = 0;
    if (Number.isFinite(planBudget) && typeof this.repository.usageSince === 'function') {
      monthSpendUsd = await this.repository.usageSince(session.sub, 'model.usd', startOfUtcMonth());
      if (monthSpendUsd >= planBudget) throw new CommercialHttpError(429, 'monthly_ai_budget_exhausted');
    }

    const compiledContext = this.promptBoundary.compile([{ source: 'user', text: goal }, ...safeContext]);
    const requestedBudget = budgetUsd === null ? this.commercialConfig.perRequestBudgetUsd : Number(budgetUsd);
    const perRequestBudget = Math.min(positiveNumberOr(requestedBudget, this.commercialConfig.perRequestBudgetUsd), this.commercialConfig.perRequestBudgetUsd);
    const remainingMonthly = Number.isFinite(planBudget) ? Math.max(0, planBudget - monthSpendUsd) : Infinity;
    const effectiveBudget = Math.min(perRequestBudget, remainingMonthly);
    if (effectiveBudget <= 0) throw new CommercialHttpError(429, 'monthly_ai_budget_exhausted');

    const result = await this.modelRuntime.generate(
      { input: goal, route, context: compiledContext },
      { preferred: this.commercialConfig.modelPreference, accountId: session.sub, budgetUsd: effectiveBudget, retries: 1 },
    );

    if (typeof this.repository.recordUsage === 'function') {
      await Promise.all([
        this.repository.recordUsage({ accountId: session.sub, metric: 'model.usd', quantity: result.costUsd, metadata: { provider: result.provider, route: route.depth } }),
        this.repository.recordUsage({ accountId: session.sub, metric: 'model.requests', quantity: 1, metadata: { provider: result.provider, route: route.depth } }),
        this.repository.recordUsage({ accountId: session.sub, metric: 'model.input_tokens', quantity: Number(result.result?.usage?.inputTokens ?? 0), metadata: { provider: result.provider } }),
        this.repository.recordUsage({ accountId: session.sub, metric: 'model.output_tokens', quantity: Number(result.result?.usage?.outputTokens ?? 0), metadata: { provider: result.provider } }),
      ]);
    }
  ...
  #issueSession(subject, claims = {}) {
    const token = this.sessions.issue({ subject, claims, ttlSeconds: 60 * 60 * 24 * 30 });
    return { token, account: { id: subject, ...claims } };
  }
}
...
function buildModelRuntime(env) {
  ...
  if (env.OPENAI_API_KEY) runtime.register('openai', new OpenAIResponsesProvider({ apiKey: env.OPENAI_API_KEY, model: env.CORTEX_OPENAI_MODEL ?? 'gpt-5.6' }), pricing(env, 'OPENAI'));
...
function pricing(env, prefix) {
  const defaults = DEFAULT_PROVIDER_PRICING[prefix] ?? { input: 0, output: 0 };
  return {
    usdPer1MInput: Number(env[`CORTEX_${prefix}_INPUT_USD_PER_1M`] ?? defaults.input),
    usdPer1MOutput: Number(env[`CORTEX_${prefix}_OUTPUT_USD_PER_1M`] ?? defaults.output),
    hosted: true,
  };
}
...
function startOfUtcMonth(now = new Date()) { return new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)); }
function optionalPositiveNumber(value) { if (value === undefined || value === null || value === '') return undefined; const number = Number(value); if (!Number.isFinite(number) || number <= 0) throw new Error('AI monthly budget must be a positive number'); return number; }
function positiveNumberOr(value, fallback) { const number = Number(value); return Number.isFinite(number) && number > 0 ? number : fallback; }
