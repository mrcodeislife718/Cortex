...
export default async function handler(req, res) {
  const origin = String(req.headers.origin ?? ''); applyCors(res, origin);
  ...
  try {
    ...
    if (action === 'activation-redeem' && req.method === 'POST') { const body = await parseJson(req); return json(res, 200, await runtime.redeemDesktopActivation(body.code)); }
    if (action === 'webhook' && req.method === 'POST') { const raw = await readRawBody(req); const result = await runtime.applyWebhook(raw.toString('utf8'), req.headers['stripe-signature']); return json(res, 200, { received: true, ...result }); }
    ...
    if (action === 'portal' && req.method === 'POST') { const appUrl = requiredEnv('CORTEX_APP_URL').replace(/\/$/, ''); return json(res, 200, await runtime.billingPortal({ session, returnUrl: `${appUrl}/account` })); }
    return json(res, 404, { error: 'not_found' });
  } catch (error) {
    const status = error instanceof CommercialHttpError ? error.status : 500; const code = error instanceof CommercialHttpError ? error.code : 'internal_error';
    if (status >= 500) console.error('cortex-commercial', error);
    return json(res, status, { error: code });
  }
}

async function getRuntime() { runtimePromise ??= CortexCommercialRuntime.fromEnvironment(); return runtimePromise; }
async function readRawBody(req) { const chunks = []; let total = 0; for await (const chunk of req) { total += chunk.length; if (total > 1024 * 1024) throw new CommercialHttpError(413, 'body_too_large'); chunks.push(chunk); } return Buffer.concat(chunks); }
async function parseJson(req) { const raw = await readRawBody(req); if (!raw.length) return {}; try { return JSON.parse(raw.toString('utf8')); } catch { throw new CommercialHttpError(400, 'invalid_json'); } }
function applyCors(res, origin) { const allowed = String(process.env.CORTEX_ALLOWED_ORIGINS ?? '').split(',').map((value) => value.trim()).filter(Boolean); if (origin && allowed.includes(origin)) { res.setHeader('Access-Control-Allow-Origin', origin); res.setHeader('Vary', 'Origin'); } res.setHeader('Access-Control-Allow-Headers', 'authorization, content-type, stripe-signature'); res.setHeader('Access-Control-Allow-Methods', 'GET,POST,OPTIONS'); res.setHeader('Cache-Control', 'no-store'); res.setHeader('X-Content-Type-Options', 'nosniff'); res.setHeader('Referrer-Policy', 'no-referrer'); }
